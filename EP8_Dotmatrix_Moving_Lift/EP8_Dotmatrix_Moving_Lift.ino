...
const unsigned char disp1[94][8]=
{
  {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},// (space)
  {0x00, 0x00, 0x00, 0x5F, 0x00, 0x00, 0x00, 0x00},// (!)  {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},// (space)
  ...
  {0x00, 0x00, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00},// (_)
  ...
  {0x00, 0x01, 0x71, 0x09, 0x05, 0x00, 0x00, 0x00},// (7)
  ...
  {0x00, 0x38, 0x44, 0x44, 0x48, 0x20, 0x00, 0x00},// (c)
  ...
  unsigned char Move[8], BufUp[8], BufDw[8];
  unsigned char Pos = 0;
  unsigned char Floor[12] = {"BG123456789"};
...
void setup()
{
  ...
  pinMode(CS, OUTPUT);
  pinMode(DIN, OUTPUT);
  pinMode(Sw_Up, INPUT);digitalWrite(Sw_Up,HIGH);
  pinMode(Sw_Dw, INPUT);digitalWrite(Sw_Dw,HIGH);
  pinMode(Buzzer, OUTPUT);digitalWrite(Buzzer,HIGH);
  delay(50);
  Init_MAX7219();
  for(i=0;i<8;i++){Move[i] = disp1[Floor[Pos]-0x20][i];}
  for(j=0;j<8;j++){for(i=0;i<8;i++){Write_Max7219(i+1,Move[1]);}}
}
...
void loop()
{
  ...
  if(digitalRead(Sw_Up) == 0)
  {
    ...
    {
      Pos = Pos+1;
      for(i=0;i<8;i++){BufUp[i] = disp1[Floor[Pos]-0x20][i];}
      for(j=0;j<8;j++)
      {
        MoveUp();
        delay(50);
        for(i=0;i<8;i++){Write_Max7219(i+1,Move[i]);}
      }
    }
  if(digitalRead(Sw_Up) == 0)
  {
  ...
  Pos = 'A' - 0x20;
  for(i=0;i<8;i++){Move[i] = disp1[Pos][i];}
  for(j=0;j<8;j++)
  {
    for(i=0;i<8;i++){Write_Max7219(i+1,Move[i]);}
    delay(200);
